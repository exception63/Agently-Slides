...
**手腕垂下、屏幕熄灭时，捏合手势不工作** —— 这是 Apple 的设计（官方支持文档明示：显示屏因手腕放下而未激活时不支持该手势），不是本 App 的问题。
现场用法是「**轻抬手腕让屏幕亮 → 捏合**」，不必真的举到眼前。

调这两项能明显扩大可用窗口：

| 设置 | 路径 | 调成 |
|---|---|---|
| 亮屏时长 | 设置 → 显示与亮度 → 唤醒时长 | **70 秒**（默认 15 秒） |
| 返回时钟 | 设置 → 通用 → 返回时钟 | **1 小时后**（或给本 App 设「始终」） |

第二项尤其关键：捏合触发的是**当前最前台 App** 的主操作；表若已跳回表盘，捏合会去操作 Smart Stack 而不是翻页。
另需确认**低电量模式 / 剧院模式 / 睡眠专注**都关着（它们会禁用抬腕唤醒与手势），「抬腕唤醒」开着，表戴紧。

> 取舍提示：既然捏合也要先抬腕，那它相对「抬腕 + 点屏幕分区」并不省动作，而且多了系统识别延迟。
> 捏合的真正好处是**不用瞄准**（不必看清按钮位置）。要「手完全垂着不动就翻页」，Apple Watch 做不到。
