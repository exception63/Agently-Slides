...
开了 **watch mode** 会多第 4 件：**提词表 `window.__SM_CUES__` + `✦ 提词` 按钮 + 提词预览窗**（见下方「Watch mode」章节）

---

## Watch mode · 手表提词（可选 · v0.4）

开启后，slides 里会多烘一个 **`✦ 提词`** 按钮和一份**按锚点索引的提词表**；
Apple Watch 上就能看到当前页的提词，抬腕零点几秒扫一眼就能接上话。

**什么时候开**：讲者会戴表上台、且希望脱稿讲。不戴表就别开——它会给讲稿作者
额外加一道「每页都要拟提词」的活。
...
window.__SM_CUES__ = {
  ...
  "s1-boom":  ["无缝嵌入"],
  "s1-hook":  ["感官剥夺"],
  // …键必须是 SLIDE_MAP 里出现过的锚点
};
```

**为什么按锚点而不是按 slide 序号**：`SLIDE_MAP` 已经建立了 slide→锚点的 1:1 映射，
而遥控协议的 `state` 消息本来就带 `anchor` 字段。按锚点存，手表链路**一行都不用改**；
按序号存，一旦中间插一页，全表错位。

### ⭐⭐⭐ 红线 · 硬约束（实测得出，别放宽）

在 Apple Watch Ultra 3（**211 × 257 点**）和 Series 11 46mm（208 × 248 点）上实测——
两者差别极小，所以**一套规则通用**。提词按 19pt 半粗排：

| 约束 | 值 | 为什么 |
|---|---|---|
| **每页必须有** | 缺一页即失败 | 讲到那页表上一片空白，比没有这个功能更慌 |
| **每页条数** | **1–5 条** | 表盘放得下 5 行短提词。**skill 出品的上限就是 5**；真要更多得在 Studio 里手动加，手表上会变成要翻页——能用，但不优雅 |
| **每条长度** | **≤10 个汉字**（英文 ≤16 字符）| 超过约 10 字就折行；实测把整句研究问题（50+ 字）加粗，直接铺满整屏，和显示讲稿全文没区别 |
| **必须是内容锚点** | 不能是结构标签 | 实测「第一部分 / 第二部分 / 第三部分 / 第四部分」这种占满一屏却什么忙也帮不上 |
| **不得与 slide 标题重复** | — | 手表顶部已经显示 `5/45 · 标题` 了，再重复一遍是浪费仅有的几行 |

⛔ **反例**（都是真讲稿里抓出来的）：

```js
"s1-question": ["AR 界面中固有的感官冲突——高度视觉真实性与零触觉反馈之间的落差——究竟会如何影响消费者的动机与行为？"],  // 50+ 字，铺满整屏
"s0-agenda":   ["第一部分","第二部分","第三部分","第四部分","AR → 感知触觉落差 → 触觉渴望 → 补偿性消费反应"],       // 条数没超，但全是结构标签、且最后一条过长
```
...
"s1-boom": ["无缝嵌入"],
"s1-hook": ["感官剥夺"],
"s1-paradox": ["虚实悖论", "补偿动机"],
```

### 来源 · 从讲稿的 `<strong>` 种子化，再逐页过一遍

**不要让作者另起一份提词表**——那是第二个真相源，必然漂移。正确做法：

1. 讲稿里作者照旧用 `<strong>` 标要点（这本来就是 Keywords 提词高亮用的东西，
   `templates/script-listener.js.template` 的 `body.cue-on` 只高亮当前板块内的 `<strong>`）
2. **按 SLIDE_MAP 的锚点分段**，抽出每段的 `<strong>` 作为该页提词的**草稿**
3. **逐页按上表校验**，不合规的当场改写 —— 这一步是人（或 AI）的判断，不能省：
   `<strong>` 在讲稿里同时担着「阅读强调」和「口播提词」两个角色，
   前者（如「第一部分」）不该出现在手表上
4. 把定稿写进 `window.__SM_CUES__`

⚠️ **实测参考**：一份 45 页的真讲稿里，27 页（60%）本来就标了 `<strong>`，
但其中约三分之一不合规（太长或是结构标签）。**别指望直接抽取就能用。**

### 校验（红线 · 缝合前必须跑）

watch mode 开启时，**逐条检查并把不合规的页全部列出来**，缺一页都不许往下走：

```
第 7 页 [s1-div]      ✗ 没有提词
第 12 页 [s1-question] ✗ 提词过长（52 字，上限 10）
第 3 页 [s0-agenda]    ✗ 疑似结构标签（「第一部分」这类在手表上帮不上忙）
```

### 出口多一件：`✦ 提词` 按钮 + 提词预览窗

注入代码用 `templates/watch-cues.js.template`（把 `{{CHANNEL}}` 换成本 deck 专属频道名）。

- slides 上多一个 `✦ 提词` 按钮（只在 watch mode 开启时烘进去）
- 点开是一个小窗，**按当前页显示提词，排版与手表一致**——
  讲者上台前能在电脑上确认「待会儿手腕上会看到什么」，不用真的举着表试
- 这个小窗跟副屏走同一条同步通道（翻页即跟随）

### 手表那一端（已就位，不用重做）

原生 app 已经能显示提词：手表左右滑第二页就是提词页。链路是
`deck → state{anchor} → 手机 → WCSession → 手表`。手表侧优先读 `__SM_CUES__`；
deck 没有提词表时，回落到运行时解析讲稿 `<strong>`（兜底，质量不保证）。
细节见 `apps/SlidesmithRemote/Shared/TranscriptNotes.swift`。
...
**开了 watch mode 的话，再加这几条**：

- [ ] `__SM_CUES__` 的**每个键都能在 SLIDE_MAP 里找到**（拼错的锚点＝那页永远没提词，且悄无声息）
- [ ] **每一页都有提词** —— 逐页数一遍，别抽查
- [ ] 每页 1–5 条 · 每条 ≤10 汉字 · 没有「第一部分」这类结构标签
- [ ] 点 `✦ 提词` 按钮 · 小窗排版与手表一致 · 翻页跟随
- [ ] 手表上真机看一眼：抬腕零点几秒能读完，不需要转表冠

---

## 红线 / 不要做的事

⛔ **开了 watch mode 就不许留空页**。手表上一片空白比没有这个功能更让人慌——
讲者会下意识低头再看一眼，那零点几秒正好卡在话头上。宁可写一个不那么妙的词，也别空着。

⛔ **不要把讲稿的 `<strong>` 直接倒进提词表**。`<strong>` 在讲稿里同时担着「阅读强调」
和「口播提词」两个角色，实测一份 45 页真讲稿里约三分之一不合规（太长、或是「第一部分」
这类结构标签）。必须逐页过一遍。
...
│   ├── watch-cues.js.template                  ⭐ watch mode 注入代码（v0.4 新增 · 提词表 + ✦提词按钮 + 表盘比例预览窗）
