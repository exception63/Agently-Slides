...
(function () {
  ...
  function startPresenterFeed() {
    ...
    //
    // ⚠️ 必须判空。猜不到频道名（deck 没烘 __SM_PRESENTER_CHANNEL__ / __SM_DECK_ID__，
    // 且 localStorage 里还没有 *-state —— 全新打开的页面就是这个状态）时 chan 是 null，
    // 直接挂 onmessage 会抛，把整个 sendDeckInfo 打断，讲稿第一次推送就丢了。
    // 表现很隐蔽：第二设备每 2 秒重问一次，第二次才拿到，看起来"只是慢了点"。
    // 上面那条 DOM 兜底（startDomWatch）本来就是为这种情况准备的，让它单独扛住即可。
    if (PFEED.chan) PFEED.chan.onmessage = function (e) {
      var d = e && e.data;
  ...
  function sendDeckInfo() {
    ...
    relaySend({
      ...
      // 现场问答：房间号 + 问答页锚点。第二设备靠这两个自动开出「问答」面板并在翻到那页时自动切过去。
      qaRoom: (typeof window.__SM_QA_ROOM__ === 'string' && window.__SM_QA_ROOM__) || null,
      qaAnchor: (typeof window.__SM_QA_ANCHOR__ === 'string' && window.__SM_QA_ANCHOR__) || null
    });
  }
  ...
  function startPairing(wsBaseHttp, phoneBase, label) {
    ...
    openDeckWs(wsBaseHttp, room, label, false);
  }
  ...
  function openDeckWs(wsBaseHttp, room, label, takeover) {
    ...
    ws.onmessage = function (e) {
      ...
      else if (m.type === 'evicted') {                            // 别的窗口显式接管了
        closePc();
        pairError('另一台设备接管了本房间的放映端。要在本窗口讲，重新点一次「手机遥控」并选择「接管」。');
      }
      else if (m.type === 'deck-busy') {                          // 房间里已经有放映端
        var msg = card.querySelector('#__sm_pairmsg');
        if (msg) {
          msg.innerHTML = '⚠️ 本房间已有放映端在线。<br>'
            + '<span style="color:#888">若<b>这一台</b>才是要讲的那台，点下面接管。</span><br>'
            + '<button id="__sm_takeover" style="margin-top:10px;border:0;background:#1c1c1f;color:#fff;'
            + 'border-radius:8px;padding:8px 18px;font-size:13.5px;cursor:pointer">接管本房间</button>';
          msg.style.color = '#8a6224';
          var tb = msg.querySelector('#__sm_takeover');
          if (tb) tb.onclick = function () {
            msg.textContent = '正在接管…'; msg.style.color = '#888';
            try { ws.close(); } catch (e2) {}
            openDeckWs(wsBaseHttp, room, label, true);
          };
        }
      }
