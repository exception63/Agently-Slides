...
## 中转怎么选：Cloudflare（默认）vs 自己的服务器

**默认是 Cloudflare，不要擅自改这个默认。** 它免费、免维护、一直用得好。
只有一种情况该切到自托管：**要给国内的学生 / 观众扫码**。

Studio 里「导出 → 嵌入手机遥控」下面有个下拉，两个选项；命令行/脚本里用
`window.__SM_CLOUD_RELAY__` 指定。**用户会明确告诉你用哪个，别自己替他决定。**

## 遥控密码（可选）

deck 里烘一行 `window.__SM_PASS__ = "<sha256 十六进制>"`，中转就会要求
**遥控端（remote / host）**出示同一个哈希才放行。

- 哈希 = `sha256("slidesmith-remote:" + 密码)`，三端（Studio / 网页 / iOS）用的是同一个盐
- **学生提问端（ask）和大屏（wall）永远不要密码** —— 加门等于把互动本身废掉
- 连错 6 次冷却 60 秒
- 没烘这一行 = 不设密码，行为和以前完全一样（旧版 iPhone app 照常连）

⚠️ **要清楚它挡的是什么**：它挡住「在台下扫了一眼大屏二维码就想遥控」的人。
真去下载公开的 deck 文件、翻出那串哈希的人仍然进得来 —— 哈希就是凭证本身，
而 deck 是公开分享的。要更强的隔离就别公开分享那一份，现场用本地文件。

## 老小节：workers.dev vs 自己的域名
