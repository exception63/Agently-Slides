...
## 一之四、全屏播放画面偏出可视区已修 —— `c745e2e`

**症状**：全屏播放点下一页，一大片灰、画面偏出去，手动拉回来翻下一页又偏。
**真因**（standalone 打开 deck 也复现 → 是 **deck 模板**的问题，不是 Studio）：
`body.present .slide` 用 `transform:scale(var(--sc))` 缩放，**但 transform 不改布局尺寸** ——
盒子在布局上仍是 1920×1080。屏幕只要比 1920×1080 小（＝所有笔记本），`.deck` 就被撑出
滚动条；flex 居中的溢出**两边对称**，右边能滚到、左边那截滚都滚不回来。
1728×899 实测：文档 1824×1080，横向溢出 96px、纵向 181px，幻灯片视觉顶端 -82px。

**修法**：负 margin 把布局盒子收回视觉尺寸
（`margin:calc((1080px*var(--sc,1)-1080px)/2) calc((1920px*var(--sc,1)-1920px)/2)`）。
改了三处：① 模板 `assets/_core.css` + `assets/deck-template.html`；
② **Studio 注进每一次 assemble（预览+导出）** —— 存量 deck 把 CSS 烤死在自己 `<style>` 里，
只改模板救不了用户手上那份；③ 仓库里 33 份成品/样例 deck 就地修掉。

**顺手修的**：`cleanSectionHtml()` 现在剥掉 `--sc` —— 它是引擎按当时那块屏算的运行时值，
以前被 `outerHTML` 烤进保存/导出的文件（用户 deck 里躺了 4 个 `--sc: 0.4619`）。

⚠️ **这类 bug 的通用教训**：`transform: scale()` 只改画面不改布局。凡是「缩放 + 居中 +
容器要正好装下」的地方，都要检查布局盒子有没有跟着收 —— 而且 **1920 宽以上的屏完全看不出来**，
必须按小尺寸测。

**回归**：`node scripts/verify-present-fit.mjs`（12/12，1280×800 / 1440×900 / 1728×1080
各翻 4 页，**自带对照组**：摘掉修复必须溢出，否则等于没测）。
