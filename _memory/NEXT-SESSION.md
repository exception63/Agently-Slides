...
## 一、下一件事：给 HTML slides 加更多形式的互动（用户点名的下一步）

底座已经全部就位，**这一轮只需要造"玩法"，不要碰中转**。

### 现成的地基（别重做）
- **自托管中转** `smrelay` 已上线 `https://live.zhouliying.com`（AWS 新加坡机，systemd，端口 8092）。
  部署脚本 / 协议 / 角色表 / 排障 / 三条现场经验全在
  `plugin/slidesmith/skills/phone-remote/relay/selfhost/README.md`，**先读它**。
- 五个角色：`deck`（放映·先到先得）· `remote`（遥控/讲稿）· `ask`（学生）· `wall`（大屏）· `host`（讲者控制台）。
- 已跑通的玩法只有一种：**扫码匿名提问 → 实时上墙 → iPad 控制**（2026-08-30 宣讲实战用）。

### 加一种新玩法要写的只有两页
学生扫的那页 + 大屏显示那页。**中转和角色一行都不用改**，消息类型自己定（照 `qa-*` 的样子起名）。
...
用户的原话是想把这些**做成菜单** —— 以后说「这门课要投票 + 词云」，AI 直接装配，不用每次现造。
所以这一轮的重点不是做完六种，而是**先把"装配"这件事的形状定下来**（一种玩法 = 一个可插拔的注入块？
还是 phone-remote 里的一组模板？），再挑一两种做样板。

### 动手前必须知道的三条（血的教训）
1. **给学生扫的码不能用 `*.workers.dev`** —— 国内移动网络打不开。自有域名。
2. **问答/投票端绝不能复用 `deck` 角色** —— 那是放映端、有占位规则，台下一点就把讲台踢下线。
3. **大屏上要滚动的区域不能用 `column-count`** —— 固定高度下它往右溢出，纵向永远滚不动。
...
| — | **现场互动（2026-08-28，不在本仓库的交付）**：企业管理培养方案宣讲 deck（31 页 academic）+ 一体版讲稿 + 扫码提问。自托管中转 `smrelay` 上线 `live.zhouliying.com`；`phone-remote` 角色扩到 5 个、deck 改先到先得 + 显式接管、iPad 加问答模式、问题落盘；修掉 `PFEED.chan` 未判空致讲稿首推丢失的既有 bug。三端联调 12/12。全套已归档到 Aboutme 的导师组模块 |
